...
// ══════════════════════════════════════════════════════
// MOBİL CİHAZLAR UYGULAMASI — SEKMELİ
// ══════════════════════════════════════════════════════
let mobilTab = 'aktif'; // 'aktif' | 'engellenenler'
let bannedDevices = [];

function switchMobilTab(tab) {
  mobilTab = tab;
  document.querySelectorAll('.mobil-tab-btn').forEach(b => {
    b.classList.toggle('active', b.dataset.tab === tab);
  });
  if (tab === 'aktif') renderMobileDevices();
  else renderBannedDevices();
}

function renderMobileDevices() {
  const el = document.getElementById('mobil-list');
  if (!el) return;
  if (mobilTab !== 'aktif') return;
  ...
  el.innerHTML = mobileDevices.map(d => {
    const locIcon  = d.locationActive
      ? '<span style="color:#00cc44;font-size:10px;">● KONUM AÇIK</span>'
      : '<span style="color:#6a8aaa;font-size:10px;">○ KONUM KAPALI</span>';
...
async function renderBannedDevices() {
  const el = document.getElementById('mobil-list');
  if (!el) return;
  el.innerHTML = '<div class="cad-empty">► Yükleniyor...</div>';
  try {
    const r    = await fetch('/api/mobile/banned');
    bannedDevices = await r.json();
    if (!bannedDevices.length) {
      el.innerHTML = '<div class="cad-empty">► Engellenen cihaz yok.</div>';
      return;
    }
    el.innerHTML = bannedDevices.map(id => `
      <div class="mobil-row">
        <div class="mobil-row-top">
          <span class="mobil-username" style="color:#ff8888;">🚫 ${escH(id)}</span>
        </div>
        <div class="mobil-actions">
          <button class="mobil-unban-btn" onclick="unbanMobileDevice('${escH(id)}')">[ ENGELİ KALDIR ]</button>
        </div>
      </div>`).join('');
  } catch {
    el.innerHTML = '<div class="cad-empty">► Yüklenemedi.</div>';
  }
}
...
async function banMobileDevice(deviceId) {
  if (!confirm(`"${deviceId}" cihazını ENGELLEMEK istiyor musunuz?`)) return;
  try {
    await fetch(`/api/mobile/ban/${encodeURIComponent(deviceId)}`, { method:'POST' });
    cadLog('MOBİL', `Cihaz engellendi: ${deviceId}`, 'sys');
    if (mobilTab === 'engellenenler') renderBannedDevices();
  } catch { alert('Hata oluştu.'); }
}

async function unbanMobileDevice(deviceId) {
  if (!confirm(`"${deviceId}" cihazının engelini kaldırmak istiyor musunuz?`)) return;
  try {
    await fetch(`/api/mobile/unban/${encodeURIComponent(deviceId)}`, { method:'POST' });
    cadLog('MOBİL', `Engel kaldırıldı: ${deviceId}`, 'sys');
    renderBannedDevices();
  } catch { alert('Hata oluştu.'); }
}

// ══════════════════════════════════════════════════════
// ÇAĞRI OLUŞTUR FORMU (PC'den mobilE)
// ══════════════════════════════════════════════════════
let cadFormView = 'list'; // 'list' | 'create'

function showCADCreateForm() {
  cadFormView = 'create';
  const listEl   = document.getElementById('cad-left');
  const createEl = document.getElementById('cad-create-panel');
...
function populateDeviceSelect() {
  const sel = document.getElementById('ccf-device-select');
  if (!sel) return;
  sel.innerHTML = '<option value="">-- Cihaz seçin --</option>';
  mobileDevices.forEach(d => {
    const opt = document.createElement('option');
    opt.value       = d.deviceId;
    opt.textContent = `📱 ${d.username}${d.locationActive ? ' ● KONUM AÇIK' : ''}`;
    sel.appendChild(opt);
  });
}

function resetCADCreateForm() {
  ['ccf-code','ccf-detail','ccf-address','ccf-sms-preview'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = el.tagName === 'TEXTAREA' ? '' : (el.id === 'ccf-sms-preview' ? '' : '');
  });
  const preview = document.getElementById('ccf-sms-preview');
  if (preview) preview.textContent = '';
  const statusEl = document.getElementById('ccf-status');
  if (statusEl) statusEl.textContent = '';
  const gpsEl = document.getElementById('ccf-gps-status');
  if (gpsEl) gpsEl.textContent = '';
  // Seçili cihazın konumunu temizle
  document.getElementById('ccf-address').value = '';
}

// Seçilen cihazın konumunu al
function getDeviceLocation() {
  const deviceId = document.getElementById('ccf-device-select').value;
  const statusEl = document.getElementById('ccf-gps-status');
  if (!deviceId) { statusEl.textContent = '► Önce cihaz seçin'; statusEl.style.color = '#ff4444'; return; }

  const device = mobileDevices.find(d => d.deviceId === deviceId);
  if (!device) { statusEl.textContent = '► Cihaz bulunamadı'; statusEl.style.color = '#ff4444'; return; }
  if (!device.locationActive || device.lat == null) {
    statusEl.textContent = '► Bu cihazın konumu kapalı'; statusEl.style.color = '#ffaa00'; return;
  }

  document.getElementById('ccf-address').value = `${device.lat.toFixed(5)}, ${device.lng.toFixed(5)}`;
  statusEl.textContent = `✅ Konum alındı (${new Date(device.locUpdatedAt).toLocaleTimeString('tr-TR')})`;
  statusEl.style.color = '#00cc44';
}

// Gemini ile SMS taslağı oluştur
let geminiLoading = false;
async function generateSmsWithGemini() {
  if (geminiLoading) return;
  const code    = document.getElementById('ccf-code').value.trim();
  const detail  = document.getElementById('ccf-detail').value.trim();
  const address = document.getElementById('ccf-address').value.trim();
  const statusEl = document.getElementById('ccf-status');
  const previewEl = document.getElementById('ccf-sms-preview');

  if (!code && !detail) {
    statusEl.textContent = '► Çağrı kodu veya açıklama giriniz'; statusEl.style.color = '#ff4444'; return;
  }

  geminiLoading = true;
  statusEl.textContent = '⏳ Gemini hazırlıyor...'; statusEl.style.color = '#ffaa00';

  const KNOWN_CODES = {
    '90':'Trafik kazası','90A':'Kaza - tek yaralı','90B':'Kaza - 3+ yaralı',
    '91':'Kavga','91A':'Kavga - yaralı','91B':'Kalabalık kavga',
    '92':'Kavga - kesici alet','93':'Kavga - ateşli silah','94':'Silahlı çatışma',
    '95':'Bayılma / travma','99':'Takip'
  };

  const knownMatch = KNOWN_CODES[code.toUpperCase()];
  let smsText = '';

  if (knownMatch && !detail) {
    // Bilinen kod, açıklama yok → hazır şablon
    const loc = address || 'Konum belirtilmedi';
    smsText = `Acil durum. ${knownMatch} yaşandı. Konum: ${loc}.`;
    statusEl.textContent = `✅ Kod ${code.toUpperCase()} — şablon kullanıldı`; statusEl.style.color = '#00cc44';
    previewEl.textContent = smsText;
    geminiLoading = false;
    return;
  }
  ...
  // Gemini ile oluştur
  try {
    const prompt = `Sen OCST (Orange County Sheriff's Technology) acil çağrı sisteminin asistanısın.
Aşağıdaki bilgilere göre 112 için kısa, net bir Türkçe SMS taslağı hazırla.
Çağrı kodu: ${code || 'Belirtilmedi'}
Açıklama: ${detail || 'Belirtilmedi'}
Adres: ${address || 'Belirtilmedi'}
Bilinen kodlar: ${JSON.stringify(KNOWN_CODES)}

Kurallar:
- Eğer kod bilinen bir kodla eşleşiyorsa o kodu kullan, uygun şablona çevir
- Eğer kod bilinmiyorsa veya serbest metin varsa, girilen açıklamayı OCST üslubuyla kısa bir acil SMS'e dönüştür
- SMS maksimum 2-3 cümle olsun
- "Acil durum." ile başlasın
- Sadece SMS metnini döndür, başka açıklama yapma`;
    ...
    const data = await r.json();
    smsText = data?.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || '';
    if (!smsText) throw new Error('Boş yanıt');
    statusEl.textContent = '✅ Gemini SMS hazırladı'; statusEl.style.color = '#00cc44';
    previewEl.textContent = smsText;
  } catch (e) {
    statusEl.textContent = '❌ Gemini hatası: ' + e.message; statusEl.style.color = '#ff4444';
  }
  geminiLoading = false;
}

async function submitCADCreate() {
  const deviceId  = document.getElementById('ccf-device-select').value;
  const code      = document.getElementById('ccf-code').value.trim();
  const detail    = document.getElementById('ccf-detail').value.trim();
  const address   = document.getElementById('ccf-address').value.trim();
  const smsText   = document.getElementById('ccf-sms-preview').textContent.trim();
  const statusEl  = document.getElementById('ccf-status');

  if (!deviceId)  { statusEl.textContent = '► Cihaz seçin'; statusEl.style.color='#ff4444'; return; }
  if (!smsText)   { statusEl.textContent = '► Önce SMS hazırlayın (Gemini butonu)'; statusEl.style.color='#ff4444'; return; }

  statusEl.textContent = '⏳ Gönderiliyor...'; statusEl.style.color = '#ffaa00';

  try {
    // 1) Çağrıyı sunucuya kaydet
    const title = code ? `📞 KOD ${code.toUpperCase()}` : '📞 PC ÇAĞRISI';
    const callRes = await fetch('/api/calls', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({
        type: 'normal', title,
        detail: detail || smsText,
    ...
    // 2) Seçili cihaza SMS komutunu ilet
    const smsRes = await fetch('/api/mobile/sms-command', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ deviceId, smsBody: smsText, callId: callData.id })
    });
    const smsData = await smsRes.json();

    if (smsData.delivered) {
      statusEl.textContent = '✅ Çağrı kaydedildi ve cihaza SMS komutu iletildi';
      statusEl.style.color = '#00cc44';
    } else {
      statusEl.textContent = '⚠️ Çağrı kaydedildi — cihaz şu an bağlı değil';
      statusEl.style.color = '#ffaa00';
    }

    cadLog('PC ÇAĞRI', `${title} — ${address||'konum yok'}`, '');
    setTimeout(() => hideCADCreateForm(), 2500);
  } catch {
    statusEl.textContent = '❌ Sunucu hatası'; statusEl.style.color = '#ff4444';
  }
}
...
function initMapApp() {
  ...
  searchBar.style.cssText = 'display:flex;align-items:center;gap:6px;background:#080d14;border-bottom:1px solid #1a3a5a;padding:6px 10px;flex-shrink:0;position:relative;z-index:500;';
  searchBar.innerHTML = `
    ...
    <input id="map-search-input" type="text" placeholder="Konum ara..." autocomplete="off" spellcheck="false"
      style="flex:1;background:rgba(0,20,50,.8);border:1px solid #1a3a5a;color:#c8d8e8;font-family:'Share Tech Mono',monospace;font-size:12px;padding:5px 10px;outline:none;letter-spacing:.5px;"
      oninput="onMapSearchInput()" onkeydown="if(event.key==='Enter')doMapSearch()"
      onfocus="this.style.borderColor='#4a9eff'" onblur="this.style.borderColor='#1a3a5a'">
    <button onclick="doMapSearch()" style="background:rgba(26,58,110,.5);border:1px solid #2a5a9a;color:#4a9eff;font-family:'Share Tech Mono',monospace;font-size:11px;padding:5px 12px;cursor:pointer;letter-spacing:1px;flex-shrink:0;">ARA</button>
    <div id="map-search-results" style="display:none;position:absolute;top:100%;left:0;right:0;z-index:9999;background:#0d1420;border:1px solid #1a3a5a;border-top:none;max-height:220px;overflow-y:auto;"></div>
  `;
...
let _mapSearchTimer = null, _mapSearchMarker = null;
...
async function doMapSearch() {
  ...
  resEl.innerHTML = `<div style="padding:8px 12px;font-size:11px;color:#6a8aaa;font-family:'Share Tech Mono',monospace;">⏳ Aranıyor...</div>`;
  try {
    const data = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&limit=7&accept-language=tr`, { headers:{'Accept-Language':'tr','User-Agent':'OCST-Desktop/4.1'} }).then(r=>r.json());
    if (!data.length) { resEl.innerHTML = `<div style="padding:8px 12px;font-size:11px;color:#6a8aaa;font-family:'Share Tech Mono',monospace;">► Sonuç bulunamadı.</div>`; return; }
    resEl.innerHTML = data.map(item => {
      const short = item.display_name.split(',').slice(0,3).join(', ');
      return `<div onclick="selectMapResult(${item.lat},${item.lon},${JSON.stringify(item.display_name)})"
        style="padding:9px 12px;cursor:pointer;border-bottom:1px solid #0f1f30;font-family:'Share Tech Mono',monospace;"
        onmouseover="this.style.background='rgba(74,158,255,.12)'" onmouseout="this.style.background='transparent'">
        <div style="font-size:12px;color:#c8d8e8;">${escH(short)}</div>
        <div style="font-size:10px;color:#4a6a8a;margin-top:2px;">${parseFloat(item.lat).toFixed(5)}, ${parseFloat(item.lon).toFixed(5)}</div>
      </div>`;
    }).join('');
  } catch { resEl.innerHTML = `<div style="padding:8px 12px;font-size:11px;color:#ff4444;font-family:'Share Tech Mono',monospace;">► Bağlantı hatası.</div>`; }
}
function selectMapResult(lat, lng, displayName) {
  ...
  const icon = L.divIcon({ className:'', html:`<div style="width:14px;height:14px;background:#ffaa00;border:3px solid #fff;border-radius:50%;box-shadow:0 0 0 3px rgba(255,170,0,.4),0 0 12px rgba(255,170,0,.8);"></div>`, iconSize:[14,14], iconAnchor:[7,7] });
  _mapSearchMarker = L.marker([lat,lng],{icon}).addTo(mapLeaflet).bindTooltip(`<div style="font-family:'Share Tech Mono',monospace;font-size:11px;">${escH(displayName.split(',').slice(0,3).join(', '))}</div>`,{permanent:false,direction:'top',className:'map-tt'}).openTooltip();
  const inp = document.getElementById('map-search-input');
  if (inp) inp.value = displayName.split(',').slice(0,3).join(', ');
}
function closeMapSearchResults() { const el = document.getElementById('map-search-results'); if (el) el.style.display = 'none'; }
document.addEventListener('click', e => { const bar = document.getElementById('map-search-bar'); if (bar && !bar.contains(e.target)) closeMapSearchResults(); });
